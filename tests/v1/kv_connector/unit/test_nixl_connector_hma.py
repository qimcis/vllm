...
@pytest.mark.cpu_test
def test_nixl_prepares_gpu_landing_only_when_it_offers_remote_tokens():
    scheduler = make_nixl_scheduler()
    scheduler.hisparse = MagicMock()
    local_request = create_request(do_remote_prefill=False)
    remote_request = create_request(do_remote_prefill=True)

    assert scheduler.get_num_new_matched_tokens(local_request, 0) == (0, False)
    scheduler.hisparse.prepare_gpu_import.assert_not_called()
    assert scheduler.get_num_new_matched_tokens(remote_request, 0) == (10, True)
    scheduler.hisparse.prepare_gpu_import.assert_called_once_with(
